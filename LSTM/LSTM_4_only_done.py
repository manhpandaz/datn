...
model = Sequential()
model.add(LSTM(units=64, activation='relu', return_sequences=True, input_shape=(
...
plt.plot(y_pred[:, 0], label='LSTM Prediction', marker="o")
...
plt.plot(y_pred[:, 1], label='LSTM Prediction', marker="o")
plt.title('USD_W - LSTM')
plt.xlabel('Time Steps')
plt.ylabel('Value')
plt.legend()
...
plt.plot(y_pred[:, 2], label='LSTM Prediction', marker="o")
plt.title('USD_W - LSTM')
plt.xlabel('Time Steps')
plt.ylabel('Value')
plt.legend()
